...
Speak("establishing connection to satellite number 545")
Speak("connection validated")
...
def MainTasksExecution(Query):
    Task = str(Query).lower()
    TasksNew = str(Query).lower()
    ReturnData = TasksExecutor(Task)

    try:
        if "open" in ReturnData:
            Value = OpenExe(TasksNew)
            return Value
    except:
        pass
